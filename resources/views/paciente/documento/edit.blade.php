        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('paciente.documento.update', $documento->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')

                        <div class="mb-4">
                            <label for="nombre" class="block text-sm font-medium text-gray-700">Nombre del Documento</label>
                            <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $documento->nombre) }}" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm">
                            @error('nombre')
